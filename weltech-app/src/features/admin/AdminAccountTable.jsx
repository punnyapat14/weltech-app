...
const AdminAccountTable = ({ roleType, title, theme }) => {
    ...
    // state เก็บสถานะการเปิดดูรหัสผ่าน (key=user_id, value=boolean)
    const [showPassMap, setShowPassMap] = useState({});

    const fetchUsers = async () => {
        setLoading(true);
        try {
            // ดึงข้อมูล profiles ตาม role
            // หมายเหตุ: password_text เป็น column ที่สมมติขึ้นตามโค้ดตัวอย่างเดิม
    ...
    return (
        <div className={`h-full flex flex-col p-2 animate-fade-in ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-2xl font-bold flex items-center gap-2">
                        {roleType === 'patient' && <User className="text-emerald-500"/>}
                        {roleType === 'doctor' && <Stethoscope className="text-blue-500"/>}
                        {roleType === 'admin' && <Shield className="text-purple-500"/>}
                        {title}
                        <span className="text-sm font-normal opacity-50 ml-2">({filteredUsers.length} ราย)</span>
                    </h2>
                </div>
                <div className="relative w-64">
                    <input 
                        ...
                        className={`w-full pl-9 pr-3 py-2 rounded-xl border outline-none text-sm transition-all focus:ring-2 ${theme === 'dark' ? 'bg-slate-800 border-slate-700 focus:ring-purple-500' : 'bg-white border-gray-200 focus:ring-blue-200'}`}
                    />
            ...
            <div className={`flex-1 overflow-hidden rounded-2xl border shadow-sm ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
                {/* ใช้ overflow-auto เพื่อให้ scroll ได้ทั้งแนวตั้งและแนวนอน */}
                <div className="overflow-auto h-full custom-scroll">
                    <table className="w-full text-left border-collapse min-w-[800px]">
                        <thead className={`sticky top-0 z-10 text-xs uppercase font-bold tracking-wider ${theme === 'dark' ? 'bg-slate-900 text-gray-400' : 'bg-gray-50 text-gray-500'}`}>
                            <tr>
                                <th className="p-4 border-b border-inherit whitespace-nowrap">ชื่อ-นามสกุล</th>
                                <th className="p-4 border-b border-inherit whitespace-nowrap">เลขบัตรประชาชน</th>
                                <th className="p-4 border-b border-inherit whitespace-nowrap">เบอร์โทรศัพท์</th>
                                <th className="p-4 border-b border-inherit text-center w-48 whitespace-nowrap">รหัสผ่าน (เดิม)</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm divide-y divide-inherit border-inherit">
                            {loading ? (
                                <tr><td colSpan="4" className="p-8 text-center opacity-50">กำลังโหลดข้อมูล...</td></tr>
                                        ...
                                        <tr key={u.id} className={`transition-colors ${theme === 'dark' ? 'hover:bg-slate-700/50 border-slate-700' : 'hover:bg-blue-50/50 border-gray-100'}`}>
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex items-center justify-center gap-2">
                                                    <div className={`flex items-center gap-1 text-xs rounded px-2 py-1 min-w-[80px] justify-center ${theme === 'dark' ? 'bg-black/30' : 'bg-gray-100'}`}>
                                                        {isShow ? (
                                ...
                                <tr><td colSpan="4" className="p-8 text-center opacity-50">ไม่พบข้อมูล</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
